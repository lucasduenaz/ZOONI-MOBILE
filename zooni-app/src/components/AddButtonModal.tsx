...
import { BotonConfig } from '../types';

interface Props {
  visible: boolean;
  onClose: () => void;
  botonesActivos: BotonConfig[];
  onAdd: (seccionKey: string) => void;
}

// Sections that can appear as home buttons (excludes 'inicio' which is always home)
const SECCIONES_DISPONIBLES = TODAS_LAS_SECCIONES.filter((s) => s.key !== 'inicio');

export default function AddButtonModal({ visible, onClose, botonesActivos, onAdd }: Props) {
            ...
            renderItem={({ item }) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  disabled={isActive}
                  accessibilityLabel={isActive ? `${item.label} ya está en Home` : `Agregar ${item.label}`}
                >
                  <Ionicons
